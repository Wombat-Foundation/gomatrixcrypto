...
Production server-key artifacts
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Build the reference mean-miner, then generate a self-signed production server
key response template. The command prints the private key and the response; do
not commit its output.

.. code-block:: bash

   (cd cuckoo/meanminer/csrc && make)
   go run ./cmd/serverkey-demo \
     -server nutra.tk \
     -valid-days 365 \
     -pow-profile production \
     -pow-max-graph-nonce 1024

Regenerate the deterministic production proof vector committed under
``serverkey/testdata/``:

.. code-block:: bash

   go run ./cmd/minting-vectors \
     -server-name nutra.tk \
     -threads 4 \
     -start-nonce 0 \
     -max-nonce 1024 \
     -output serverkey/testdata/msc00e4-nutra.tk-sha3-256-cogen-42-29-v1.json

``res/sample-bundle-nutra.tk.json`` is an illustrative, low-difficulty legacy
sample; it is not generated by either command and is not a production v1
response template.
